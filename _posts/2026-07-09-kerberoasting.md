...
tags: [active-directory, kerberos, netexec, impacket, hashcat]
excerpt: "One low-privilege Active Directory account turned into three by abusing how Kerberos hands out service tickets. Standard, targeted, and AS-REP chained roasting on a Hack Smarter lab."
permalink: /writeups/kerberoasting/
---

This Hack Smarter lab starts with a set of low-privilege Active Directory credentials, the kind you might pick up early in an engagement, and the goal of turning that one weak account into more by abusing how Kerberos hands out service tickets.
...
![Impacket GetUserSPNs listing the eve_marketing account](/assets/images/kerberoasting/1.png)

*One account, `eve_marketing`, with the SPN `HTTP/marketing.hacksmarter.local`*

The tool then prints the ticket straight to the terminal.

![Impacket printing the krb5tgs hash for eve_marketing](/assets/images/kerberoasting/2.png)

*The `$krb5tgs$23$` string is the Kerberoast hash. The `$23$` confirms RC4, which is the format Hashcat expects for mode 13100.*
...
![NetExec kerberoasting over LDAP and saving the hash to a file](/assets/images/kerberoasting/3.png)

*Same account, same ticket, now saved to `hashes.txt`*
...
![Hashcat cracking the eve_marketing ticket to reveal the password 123456](/assets/images/kerberoasting/4.png)
...
![NetExec running a targeted kerberoast against jack_exec](/assets/images/kerberoasting/5.png)

*NetExec reports `No entries found!` for the standing SPN enumeration because `jack_exec` has no permanent SPN, but it still applies the temporary SPN, captures the ticket, and writes it to the output file*

Reading the output file confirms the Kerberos hash for `jack_exec`.

![The targeted_kerberoast.txt file holding the jack_exec hash](/assets/images/kerberoasting/6.png)

*The captured hash for `jack_exec`, ready to crack*
...
![Hashcat cracking the jack_exec ticket](/assets/images/kerberoasting/7.png)
...
![NetExec enumerating 16 domain users over a null SMB session](/assets/images/kerberoasting/8.png)

*Sixteen domain users written to `usernames.txt`*
...
![NetExec AS-REP roasting the user list and returning the backup_svc hash](/assets/images/kerberoasting/9.png)

*One account comes back `KDC_ERR_CLIENT_REVOKED`, meaning its credentials are disabled, and `backup_svc` returns a `$krb5asrep$23$` hash*

The `backup_svc` account has preauthentication turned off. You can also confirm the exposure and pull Kerberoast tickets directly through the no-preauth account, which is the full chain in one command:
...
![Hashcat cracking the backup_svc AS-REP hash to reveal the password qwerty](/assets/images/kerberoasting/10.png)

*The `backup_svc` password is `qwerty`*

From there a valid credential gets a TGT, and the TGT opens up standard Kerberoasting across the rest of the domain.
