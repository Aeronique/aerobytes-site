...
Opening that folder confirms the `lsass.DMP` file is there and ready to move to our attack machine, which we'll get to in a bit.
...
ProcDump isn't installed by default, so grab it from the [Microsoft download page](https://learn.microsoft.com/en-us/sysinternals/downloads/procdump) first.
