...
This came out of the AI Cyber Defense Ops course I am taking through Just Hacking Training, which is sponsored by WiCyS. The section had us build defensive tooling, so I put together a Sysmon parser that generates sample events and tests, a Hayabusa MCP, and a detection MCP. Then I wanted to go further and build something offensive I had been meaning to try. I have used MCP integrations in plenty of tools and customized parts of popular ones to fit what I needed, but it never occurred to me that I could build my own from scratch until I sat down and did it.

The tool is [`llm-redteam-mcp`](https://github.com/Aeronique/llm-redteam-mcp), a small [Model Context Protocol](https://modelcontextprotocol.io) server that sends tricky prompts to a language model and scores how it responds. I run a few models locally through Ollama, and before I trust any of them in an agent I want to see how they hold up when someone tries to get around their guardrails. It only ever talks to my local Ollama and has no way to reach anything hosted or third party.

I paired with Claude Code and kept every prompt I used, cleaned up for readability, so you can see how the build went. The code was the easy part to get help with, and the security judgment behind the tool stayed mine. Working with a coding agent turned out to be its own skill, since I had to decide how much to ask for at once, when to hand off a whole piece versus a single function, and when to make it stop and give me a plan first.
